...
summary_OSCE <- combined_data_clean %>% 
    select(avg_bidders_per_project, avg_ratio_between_winner_and_publicado, perc_single_bidder, 
...
summary_OSCE_tipomuni <- combined_data_clean %>% 
    select(tipo_municipalidad, avg_bidders_per_project, avg_ratio_between_winner_and_publicado, perc_single_bidder, 
           perc_repeated_winners, perc_valor_adj_del_total) %>% 
...
summary_MEF <- combined_data_clean %>% 
    select(perc_proyectos_debajo_cutoff, perc_proyectos_sobrecosto) %>% 
    skim()
...
summary_MEF_tipomuni <- combined_data_clean %>% 
    select(tipo_municipalidad, perc_proyectos_debajo_cutoff, perc_proyectos_sobrecosto) %>% 
    group_by(tipo_municipalidad) %>% 
    skim()
...
combined_data_clean %>% 
    pivot_longer(cols = -c(gobierno, mesanho_publicacion, tipo_municipalidad)) %>% 
    group_by(mesanho_publicacion, name) %>% 
    summarise(
...
combined_data_clean %>% 
    pivot_longer(cols = -c(gobierno, mesanho_publicacion, tipo_municipalidad)) %>% 
    group_by(mesanho_publicacion, name, tipo_municipalidad) %>% 
    summarise(
        mean_indices = mean(value, na.rm = TRUE)
    ) %>% 
    ggplot(aes(x = mesanho_publicacion, y = mean_indices)) +
    ...
    labs(x = NULL,
         y = "Índicadores de Riesgo de Corrupción") +
    facet_wrap(~name + tipo_municipalidad, ncol = 6, scales = "free_y") 

# Factor indices
combined_data_clean %>%
    filter(mean_indices < quantile(mean_indices, probs = 0.99),
           mean_indices > quantile(mean_indices, probs = 0.01)) %>%
    select(gobierno, mesanho_publicacion, F1, F2) %>%
    pivot_longer(cols = -c(gobierno, mesanho_publicacion)) %>%
    group_by(mesanho_publicacion, name) %>%
    summarise(
        mean_indices = mean(value, na.rm = TRUE),
        n = sum(!is.na(value)),
    ...
    geom_smooth(data = . %>% filter(mesanho_publicacion > as.Date("2015-03-01")),
                method = "loess", se = FALSE, color = "red") +
    geom_ribbon(aes(ymin = mean_indices - sem, ymax = mean_indices + sem), alpha = 0.5, fill = "orange") +
    geom_vline(xintercept = as.Date("2015-03-01")) +
    labs(x = NULL, y = "Índicadores de Riesgo de Corrupción") +
...
combined_data_clean %>%
    filter(mean_indices < quantile(mean_indices, probs = 0.99),
           mean_indices > quantile(mean_indices, probs = 0.01),
           mesanho_publicacion > "2011-03-01") %>%
    select(gobierno, mesanho_publicacion, tipo_municipalidad, F1, F2) %>%
    pivot_longer(cols = -c(gobierno, mesanho_publicacion, tipo_municipalidad)) %>%
    group_by(mesanho_publicacion, name, tipo_municipalidad) %>%
    summarise(
        mean_indices = mean(value, na.rm = TRUE),
    ...
    geom_vline(xintercept = as.Date("2015-03-01")) +
    labs(x = NULL, y = "Índicadores de Riesgo de Corrupción") +
    facet_wrap(~name + tipo_municipalidad, ncol = 3, scales = "free_y")
...
correlation_analysis <- combined_data_clean %>% 
    select(-c(gobierno:tipo_municipalidad)) %>% 
