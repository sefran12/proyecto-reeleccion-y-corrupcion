...
# read overall model for factors
factor_model <- read_rds("data/03_model/2_factor_model.rds")
...
combined_data <- osce_month %>%
    left_join(osce_semester, by = c("mesanho_publicacion", "gobierno", "OBJETO"), relationship = "many-to-many")
...
combined_data_clean <- combined_data %>%
    ...
    select(gobierno, mesanho_publicacion, tipo_municipalidad, OBJETO, avg_bidders_per_project, avg_ratio_between_winner_and_publicado,
...
cfa_scores <- predict(factor_model, combined_data_clean)
...
model_df <- combined_data_clean %>% 
    ...
    group_by(gobierno, mesanho_publicacion, tipo_municipalidad, OBJETO) %>%
    slice(1) %>%
    ungroup() %>% 
    mutate(
        gobierno_objeto = str_c(gobierno, OBJETO),
        ...
        numero_efectivo_partidos_categoy = case_when(numero_efectivo_partidos < 1 ~ "]0, 1]",
    ...
    select(
        gobierno, gobierno_objeto, mesanho_publicacion, OBJETO, time_var, tipo_municipalidad, cutoff,
...
# save
write_parquet(combined_data_clean, "data/02_intermediate/cleaned_data_objeto.parquet")
write_parquet(model_df, "data/02_intermediate/model_data_objeto.parquet")
