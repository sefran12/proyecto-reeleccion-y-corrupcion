...
average_bidder_count <- combined_df_postores %>%
    group_by(mesanho_publicacion, gobierno, nombre_proceso, OBJETO) %>%
...
# save as parquet
write_parquet(indices_df, "data/02_intermediate/OSCE/monthly_indices_2017_objeto.parquet")
write.csv(indices_df, "data/02_intermediate/OSCE/monthly_indices_2017_objeto.csv")
...
total_value <- combined_df_adjudicaciones %>%
    group_by(gobierno, semester, OBJETO) %>% 
    arrange(gobierno, semester, OBJETO) %>% 
...
coberture_at_80_index <- combined_df_adjudicaciones %>%
    group_by(gobierno, semester, OBJETO) %>%
    arrange(gobierno, semester, OBJETO, desc(monto_total)) %>% 
...
# Define a function to get the overlap (repetition
# of postores
...
semestral_df %>% 
    ...
    group_by(semester, tipo_municipalidad, OBJETO) %>% 
    summarise(
...
# write 
write_parquet(semestral_df, "data/02_intermediate/OSCE/semestral_indices_2017_objeto.parquet")
