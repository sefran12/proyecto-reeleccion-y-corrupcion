...
sobrecostos <- combined_df_adjudicaciones %>%
    mutate(Publication_Year = floor_date(Publication_Date, unit = "month")) %>%
    filter(Publication_Year > as.Date("2009-12-31", format = "%Y-%m-%d")) %>%  
    mutate(overcost = Adjudicated_Value_Item - Reference_Value_Item,
           overcost_flag = ifelse(overcost > 0, 1, 0),
           overcost_percentage = ifelse(overcost > 0, overcost / Reference_Value_Item * 100, 0)) %>%
...
(plot2) + plot_annotation("Proporción de adjudicaciones con sobrecostos")
...
plot2 <- ggplot(muni_summary, aes(x = Publication_Year, y = Mean_Avg_Time_Publication_Buenapro)) +
    ...
    labs(title = NULL,
         x = "Año",
         y = "Diferencia promedio en días")

(plot2) + plot_annotation("Dias promedio para municipalidades y gobiernos regionales entre la publicación del proceso y la buena pro")
